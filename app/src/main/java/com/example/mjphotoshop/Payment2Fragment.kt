...
class Payment2Fragment : Fragment() {
    ...
    var imgpayment3: ImageView? = null
    var file: File? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        var root  =  inflater.inflate(R.layout.fragment_payment2, container, false)

        val policy = StrictMode.ThreadPolicy.Builder().permitAll().build()
        StrictMode.setThreadPolicy(policy)
        val bundle =this.arguments
        bookingID = bundle?.get("bookingID").toString()

        val sharedPrefer = requireContext().getSharedPreferences(
            LoginActivity().appPreference, Context.MODE_PRIVATE)
        userID = sharedPrefer?.getString(LoginActivity().userIdPreference, null)

        idbooking = bundle?.get("idbooking").toString()

        btnpayment2 = root.findViewById(R.id.btnpayment2)
        btnpayment2?.setOnClickListener {
            addpayment2()
            up()
        ...
        permission()
        imgpayment3 = root.findViewById(R.id.imgpayment3)
        imageViewSlip = root.findViewById(R.id.imageViewSlip)
        imgpayment3?.setOnClickListener {
    ...
    private fun addpayment2()
    {

        var url: String = getString(R.string.root_url) + getString(R.string.AddDeposit_url)
        Log.d("AddDeposit",url)
        val okHttpClient = OkHttpClient()
        val formBody: RequestBody = MultipartBody.Builder()
            .setType(MultipartBody.FORM)
            .addFormDataPart("bookingID",idbooking.toString())
    ...
    private fun up()
    {
        var url: String = getString(R.string.root_url) + getString(R.string.up_url)+idbooking
        val okHttpClient = OkHttpClient()
